...
PUBLIC int left90(char *filename) {
	...
	PIXEL **image, **left90_image;

	//p and k are assisting variables to easily bypass padding in the input file.
	byte p;
	...
	for (i = 0; i < info_header->biHeight; i++) {
		/* Allocating memory for each row of the rotated image */
		if ((left90_image[i] = (PIXEL *) malloc(
				sizeof(PIXEL) * (info_header->biWidth))) == NULL ) {
			printf("**Error: Not enough memory allocated**\n");
			return EXIT_FAILURE;
		}
		...
		for (j = 0; j < info_header->biWidth; j++) {
			left90_image[i][j] = image[info_header->biWidth - 1 - j][i];
		}

		/* Writing the pixels of the line into the new image with validation check */
		if ((fwrite(left90_image[i], sizeof(PIXEL) * info_header->biWidth, 1,
				fp_out)) < 1) {
			printf("**Error: Data omissions while writing in file %s**\n",
					out_filename);
			fclose(fp_out);
 ...
 * Function: openFile
 * 	This function sends a non-existing file name into left90() function
 * 	to test if the file - opening check is working properly.
 *
 * Function: readFile
 * 	This function creates an empty file and sends it to left90() function
 * 	to test if the file - reading checks are working properly.
 *
 * Function: isBm
 * 	This function creates a new image with type OK and sends it to left90()
 ...
 * 	to left90() function to test if the image - checking is working properly
 *
 * Function: isPixel24Bit
 * 	This function creates a new image with a color depth different than 24
 * 	and sends it to left90() function to test if the image - checking is working
 * 	properly.
 *
 * Function: isWorking
 * 	This function creates a new image with the appropriate type, compression and
 * 	color depth and sends it to left90() function to test if the function is working
...
int main() {
	...
	if((rep_file = fopen("left90_report.txt", "w+")) == NULL) {
	...
	fprintf(rep_file, "======================================\n");
	fprintf(rep_file, "** Report for left90.c test drivers **\n");
	fprintf(rep_file, "======================================\n\n");
	fprintf(rep_file, "#  Driver's Name \t\t\tStatus\n");
	fprintf(rep_file, "-----------------------------------------------\n");

	printf("\n\n*********************************\n");
	printf("   <BEGIN of left90.c Testing>\n");
	printf("*********************************\n\n");
	...
	printf("\n\n********************************\n");
	printf("   </END of left90.c Testing>\n");
	printf("********************************\n\n");
