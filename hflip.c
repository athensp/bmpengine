...
PUBLIC int hflip(char *filename) {
	...
	// Index when we are going through the image's pixels.
	dword i, j;
	...
	for (i = info_header->biHeight; i > 0; i--) {
		/* Allocating memory for the image's rows in the image array */
		if ((image[i - 1] = (PIXEL *) malloc(
		...
		if ((fread(image[i - 1], sizeof(PIXEL) * info_header->biWidth, 1, fp_in))
				< 1) {
			printf("**Error: Information missing from file %s**\n", filename);
			fclose(fp_in);
			return EXIT_FAILURE;
	...
	for (i = 0; i < info_header->biHeight; i++) {
		...
		/* Adding the padding into the new image */
		for (j = 0; j < padding; j++) {
			fputc(0x00, fp_out);
		}

		/* Freeing the image's i-th row */
		free(image[i]);
	}
 ...
 * Function: openFile
 * 	This function sends a non-existing file name into hflip() function
 * 	to test if the file - opening check is working properly.
 *
 * Function: readFile
 * 	This function creates an empty file and sends it to hflip() function
 * 	to test if the file - reading checks are working properly.
 *
 * Function: isBm
 * 	This function creates a new image with type OK and sends it to hflip()
 ...
 * 	to hflip() function to test if the image - checking is working properly
 *
 * Function: isPixel24Bit
 * 	This function creates a new image with a color depth different than 24
 * 	and sends it to hflip() function to test if the image - checking is working
 * 	properly.
 *
 * Function: isWorking
 * 	This function creates a new image with the appropriate type, compression and
 * 	color depth and sends it to hflip() function to test if the function is working
...
int main() {
	...
	if((rep_file = fopen("hflip_report.txt", "w+")) == NULL) {
	...
	fprintf(rep_file, "=====================================\n");
	fprintf(rep_file, "** Report for hflip.c test drivers **\n");
	fprintf(rep_file, "=====================================\n\n");
	fprintf(rep_file, "#  Driver's Name \t\t\tStatus\n");
	fprintf(rep_file, "-----------------------------------------------\n");

	printf("\n\n*******************************\n");
	printf("   <BEGIN of hflip.c Testing>\n");
	printf("*******************************\n\n");
