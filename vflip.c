...
int vflip(char *filename) {
	...
	// 2d arrays that will represent the image
	// and a temporary pixel to assist the
	// swapping of pixels.
	PIXEL **image = NULL, temp_pixel;
	...
	/* Calculating the padding of the image */
	...
	for (i = 0; i < info_header->biHeight; i++) {
		...
		/* Swapping the pixels by using a simple swapping method */
		for (j = 0; j < (info_header->biWidth / 2); j++) {
			temp_pixel = image[i][j];
			image[i][j] = image[i][info_header->biWidth - j - 1];
			image[i][info_header->biWidth - 1 - j] = temp_pixel;
		}
		...
		/* Skipping padding in the input file and placing it into the output file */
		for (k = 0; k < padding; k++) {
			p = fgetc(fp_in);
			fputc(0x00, fp_out);
		}
		/*Free the image's row*/
		free(image[i]);
	}
 ...
 * Function: openFile
 * 	This function sends a non-existing file name into vflip() function
 * 	to test if the file - opening check is working properly.
 *
 * Function: readFile
 * 	This function creates an empty file and sends it to vflip() function
 * 	to test if the file - reading checks are working properly.
 *
 * Function: isBm
 * 	This function creates a new image with type OK and sends it to vflip()
 ...
 * 	to vflip() function to test if the image - checking is working properly
 *
 * Function: isPixel24Bit
 * 	This function creates a new image with a color depth different than 24
 * 	and sends it to vflip() function to test if the image - checking is working
 * 	properly.
 *
 * Function: isWorking
 * 	This function creates a new image with the appropriate type, compression and
 * 	color depth and sends it to vflip() function to test if the function is working
...
int main() {
	...
	if((rep_file = fopen("vflip_report.txt", "w+")) == NULL) {
	...
	fprintf(rep_file, "=====================================\n");
	fprintf(rep_file, "** Report for vflip.c test drivers **\n");
	fprintf(rep_file, "=====================================\n\n");
	fprintf(rep_file, "#  Driver's Name \t\t\tStatus\n");
	fprintf(rep_file, "-----------------------------------------------\n");

	printf("\n\n*******************************\n");
	printf("   <BEGIN of vflip.c Testing>\n");
	printf("*******************************\n\n");
